...
show_store_union_struct_prototype(sensor_map_button)
...
DEVICE_ATTR(rezerocmd, RMI_WO_ATTR,
		rmi_show_error,
		f19_rezero_store);
	...
	struct f19_data *f19)
{
	...
	if (retval < 0) {
		dev_err(&rmi_dev->dev, "%s : Could not read control reg0 to 0x%x\n",
	...
	if (retval < 0) {
		dev_err(&rmi_dev->dev, "%s : Could not read control reg1 to 0x%x\n",
	...
	if (retval < 0) {
		dev_err(&rmi_dev->dev, "%s : Could not read control reg2 to 0x%x\n",
	...
	if (retval < 0) {
		dev_err(&rmi_dev->dev, "%s : Could not read control reg3 to 0x%x\n",
	...
	if (query->has_sensitivity_adjust) {
		...
		if (retval < 0) {
			dev_err(&rmi_dev->dev, "%s : Could not read control reg5 to 0x%x\n", __func__,
	...
	if (query->has_hysteresis_threshold) {
		...
		if (retval < 0) {
			dev_err(&rmi_dev->dev, "%s : Could not read control reg6 to 0x%x\n", __func__,
...
static int rmi_f19_alloc_memory(struct rmi_function_container *fc)
{
	...
	f19->button_count = f19->query.button_count;
	...
	f19->control.reg_3->regs =
			kzalloc(f19->button_count, GFP_KERNEL);
	...
	f19->control.reg_3->length = f19->button_count;
	ctrl_base_addr += f19->button_count;
	...
	f19->control.reg_4->length = f19->button_count;
	ctrl_base_addr += f19->button_count;
...
static int rmi_f19_create_sysfs(struct rmi_function_container *fc)
{
	...
	if (query->has_hysteresis_threshold) {
		if (sysfs_create_file(&fc->dev.kobj, attrify(hysteresis_threshold)) < 0) {
			dev_err(&fc->dev, "Failed to create control sysfs files.");
			return -ENODEV;
		}
	}
...
static int rmi_f19_config(struct rmi_function_container *fc)
{
	struct f19_data *data = fc->data;
	int retval = 0;
	union f19_0d_query *query = &data->query;
	struct f19_0d_control *control = &data->control;

	retval = rmi_write_block(fc->rmi_dev, control->reg_0->address,
			(u8 *)control->reg_0->regs,
			sizeof(u8));
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write control reg0 to 0x%x\n",
				__func__, control->reg_0->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, control->reg_1->address,
			(u8 *)control->reg_1->regs, data->button_bitmask_size);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write control reg1 to 0x%x\n",
			 __func__, control->reg_1->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, control->reg_2->address,
			(u8 *)control->reg_2->regs, data->button_bitmask_size);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write control reg2 to 0x%x\n",
			 __func__, control->reg_2->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, control->reg_3->address,
			(u8 *)control->reg_3->regs, data->button_count);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write control reg3 to 0x%x\n",
			 __func__, control->reg_3->address);
		return retval;
	}

	retval = rmi_write_block(fc->rmi_dev, control->reg_4->address,
			(u8 *)control->reg_4->regs, data->button_count);
	if (retval < 0) {
		dev_err(&fc->dev, "%s : Could not write control reg3 to 0x%x\n",
			 __func__, control->reg_4->address);
		return retval;
	...
	if (query->has_sensitivity_adjust) {
		retval = rmi_write_block(fc->rmi_dev, control->reg_5->address,
				...
				sizeof(control->reg_5->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write control reg5 to 0x%x\n", __func__,
				control->reg_5->address);
			return retval;
		}
	}
	if (query->has_hysteresis_threshold) {
		retval = rmi_write_block(fc->rmi_dev, control->reg_6->address,
				(u8 *)control->reg_6->regs, sizeof(control->reg_6->regs));
		if (retval < 0) {
			dev_err(&fc->dev, "%s : Could not write control reg6 to 0x%x\n", __func__,
				control->reg_6->address);
			return retval;
		}
	}
