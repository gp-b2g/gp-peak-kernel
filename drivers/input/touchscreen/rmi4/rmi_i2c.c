				  ...
				  const struct i2c_device_id *id)
{
    ...
    //dbg_printk("entry:%s,devic_name[%s]\n",client->dev.kobj.name,dev_name(&client->dev));
	if (!pdata) {
	...
	if (error) {
        ...
        if(pdata->gpio_unconfig)
         pdata->gpio_unconfig(pdata->gpio_data);
        dbg_printk("s2202 ic not connect at addrress %0x\n",client->addr);
		goto err_data;
	}
