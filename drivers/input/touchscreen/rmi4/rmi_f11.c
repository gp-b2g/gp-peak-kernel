...
//#define __USER_KERNEL_KEY__
//#define __GPIO_CONFIG_CELLON_PRJ_C8681__
#ifdef __GPIO_CONFIG_CELLON_PRJ_C8681__
#include <asm/gpio.h>
#endif
...
#define  __SCREEN_WIDTH_MAX 540
#define __SCREEN_HEIGHT_MAX 960
#else
#define  __SCREEN_WIDTH_MAX 480
#define __SCREEN_HEIGHT_MAX 800
#endif
...
static ssize_t f11_gpio_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
        int i ,value;
...
static struct device_attribute attrs[] = {
	__ATTR(flip, RMI_RW_ATTR, f11_flip_show, f11_flip_store),
     #ifdef __GPIO_CONFIG_CELLON_PRJ_C8681__
    __ATTR(gpio_test, RMI_RW_ATTR, f11_gpio_show, NULL),
    #endif
 ...
 */
#if 0
#define TP_DEBUG(fmt, args...)       \
            printk("rmi: " fmt, ##args)
#else
#define TP_DEBUG(fmt, args...)
#endif
int flags = 0;
#define S2202_KEY_DOWN 1
#define ST_KEY_SEARCH 	217
static inline int report_key(int x, int y)
{
	int key;
	static int last_key;

   // printk("rmi: flags = %d x = %d y = %d.\n", flags, x, y);
	...
	if ((y > 1001) && (y <= 1082)) {
		...
		if (last_key == key) {
				TP_DEBUG("Skip key %s.\n", (last_key == KEY_MENU)? "MENU" :
							    ((last_key == KEY_HOME) ? "HOME" :
							    ((last_key == KEY_BACK) ? "BACK" : "INVALID")));
			return true;
        }

		/* last key is not release, but a new key come in, release the last key */
		if (unlikely(last_key)) {
			if(flags & S2202_KEY_DOWN) {
//				TP_DEBUG("Last key release %s.\n", (last_key == KEY_MENU)? "MENU" :
//							    ((last_key == KEY_HOME) ? "HOME" :
//							    ((last_key == KEY_BACK) ? "BACK" : "SEARCH")));
				input_report_key(simulate_key_input, last_key, 0);
				input_sync(simulate_key_input);

				last_key = 0;
				flags &= S2202_KEY_DOWN ;
			}
		}
				   ...
				   struct f11_2d_sensor *sensor)
{
	...
	for (i = 0, finger_pressed_count = 0; i < sensor->nbr_fingers; i++) {
		...
		if (sensor->data.abs_pos){
            ...
            //dbg_printk("report abs_pos\n");
        }
		if (sensor->data.rel_pos){
			rmi_f11_rel_pos_report(sensor, i);
            //dbg_printk("report rel_pos\n");
        }
	}
    //dbg_printk("report key\n");
...
//			y_min, y_max, 0, 0);
	...
	ret = input_register_device(simulate_key_input);
	if (ret != 0) {
