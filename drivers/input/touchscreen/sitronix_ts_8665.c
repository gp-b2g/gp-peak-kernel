...
static inline int report_key(struct point *point, int which_finger) 
{
	...
	if ((point[which_finger].y > 480) && (point[which_finger].y <= 520)) {
		if((point[which_finger].x >= 15) && (point[which_finger].x <= 65)) 
			key = KEY_MENU;
		
		else if((point[which_finger].x >= 95) && (point[which_finger].x <= 145))
			key = KEY_HOME;
		
		else if((point[which_finger].x >= 175) && (point[which_finger].x <= 225))
			key = KEY_BACK;
		
		else if((point[which_finger].x >= 255) && (point[which_finger].x <= 305))
			key = ST_KEY_SEARCH;
		else
			goto release_key;
...
release_key:
	if((flags & (which_finger == 0 ? FLG_FINGER1_KEY_DOWN : FLG_FINGER2_KEY_DOWN)) && (!point[which_finger].valid)) {
...
void report_point(struct sitronix_ts_priv *priv)
{
	...
	struct point point[2] = {{0,0,0},{0,0,0}};
...
static int __init sitronix_ts_init(void)
{
	...
	dev_t devno;
	int ret;
	gpio_request(FLAG_GPIO, "flag_sitronix");
