...
static bool compass_good = false;
	...
	int size)
{
	...
	dev_vdbg(&akm->i2c->dev, "timeout = %d\n", err);
	if (err < 0) {
		dev_err(&akm->i2c->dev,
...
AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_SET_MODE:
		dev_vdbg(&akm->i2c->dev, "set div to 0.\n");
		div = 0;
		if (argp == NULL) {
			dev_err(&akm->i2c->dev, "invalid argument.");
			return -EINVAL;
	...
	case ECS_IOCTL_WRITE:
		...
		break;
	case ECS_IOCTL_SET_MODE:
		dev_vdbg(&akm->i2c->dev, "IOCTL_SET_MODE called. PID = %d\n", current->pid);
		ret = AKECS_SetMode(akm, mode);
		if (ret < 0)
			return ret;
		break;
	case ECS_IOCTL_GETDATA:
		dev_vdbg(&akm->i2c->dev, "IOCTL_GETDATA called. PID = %d", current->pid);
	...
	struct akm8963_data *akm, char const *buf, size_t count, int pos)
{
	...
	dev_vdbg(&akm->i2c->dev, "set enable_flag [%d] -> \n", akm->enable_flag );
	mutex_lock(&akm->val_mutex);
	akm->enable_flag &= ~(1<<pos);
	akm->enable_flag |= ((uint32_t)(en))<<pos;
	mutex_unlock(&akm->val_mutex);

	dev_vdbg(&akm->i2c->dev, "[%d]\n", akm->enable_flag );
...
static irqreturn_t akm8963_irq(int irq, void *handle)
{
	...
	dev_vdbg(&akm->i2c->dev, "interrupt\n");
...
//gsh add start
bool  akm8963_sensor_id(void)
{
	return compass_good;
}
EXPORT_SYMBOL(akm8963_sensor_id); 
//gsh add end
