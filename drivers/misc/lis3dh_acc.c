...
static bool lis3dh_chip_id = false; //gsh add
		...
		const struct i2c_device_id *id)
{
	...
	*/
	...
	acc->client = client;
	i2c_set_clientdata(client, acc); // client->dev->p->drive_data = acc
...
//gsh add start
bool  lis3dh_sensor_id(void)
{
	return lis3dh_chip_id;
}
EXPORT_SYMBOL(lis3dh_sensor_id);
//gsh add end
