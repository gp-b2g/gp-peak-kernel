...
/**************************************************************
	[Start Streaming]
	BITFIELD=0x301A, 0x200, 1	//MASK_BAD_Frames
	BITFIELD=0x301A, 0x400, 1	//Restart_bad_frames
	BITFIELD=0x301A, 0x8, 1		//Lock_Register
	REG=0x0104, 0x00		//GROUPED_PARAMETER_HOLD
	BITFIELD=0x301A, 0x4, 1		//Start_Streaming
**************************************************************/
//	int32_t rc = 0;
	uint16_t reg_data;
	printk("##### %s: E\n",__func__);
	msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
		0x301A, &reg_data, MSM_CAMERA_I2C_WORD_DATA);
	printk("##### %s line%d: reg_data == 0x%x\n",__func__,__LINE__,reg_data);
	reg_data |= (1 << 9);
	printk("##### %s line%d: reg_data == 0x%x\n",__func__,__LINE__,reg_data);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x301A, reg_data, MSM_CAMERA_I2C_WORD_DATA);
...
/**************************************************************
	[Stop Streaming]
	BITFIELD=0x301A, 0x4, 0		//Start_Streaming
	BITFIELD=0x301A, 0x8, 0		//Lock_Register
	REG=0x0104, 0x01		//GROUPED_PARAMETER_HOLD
**************************************************************/
	uint16_t reg_data;
	printk("##### %s: E\n",__func__);
	msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
		0x301A, &reg_data, MSM_CAMERA_I2C_BYTE_DATA);
	reg_data &= ~(1 << 2);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x301A, reg_data, MSM_CAMERA_I2C_BYTE_DATA);
