...
#endif
#define     AUTO_CHANGE_CHARGING_CURRENT  0
#define	BATTERY_RPC_PROG		0x30000089
#define	BATTERY_RPC_VER_5_1		0x00050001

#define	BATTERY_REGISTER_PROC		2
#define	BATTERY_DEREGISTER_CLIENT_PROC	5
#define     ONCRPC_VBATT_PARAMETER_WRITE_PROC              17
...
static void msm_rpc_send_charging_flag(void);	//alex.lu add 
	
#endif
enum {
	POWER_SUPPLY_MANUFACTURER_FAILD = 0,
	POWER_SUPPLY_MANUFACTURER_TURE,
	POWER_SUPPLY_MANUFACTURER_UNKNOWN,
};
...
/*the macro  AUTO_CHANGE_CHARGING_CURRENT can enable(1)/disable(0) transfer the parameter from 
AP to BP,if you will closed the function ,you can set the AUTO_CHANGE_CHARGING_CURRENT is 0,and in modem
you can undefine FEATURE_CHG_TASK_AUTO_CHANGE_CHARGING_CURRENT 
in amss\modem_proc\build\cust\Custpmic3.h*/

#if AUTO_CHANGE_CHARGING_CURRENT
struct rpc_reply_charging_parameter {
	
	u32 change_current_parameter_1;
	u32 change_current_parameter_2;	
	u32 change_current_parameter_3;
	u32 change_current_parameter_4;
	u32 change_current_parameter_5;
	u32 change_current_parameter_6;
		
};

struct rpc_reply_charging_parameter rpc_reply_charging_parameter = {
	
	.change_current_parameter_1 =0 ,
	.change_current_parameter_2 =0 ,
	.change_current_parameter_3 =0 ,
	.change_current_parameter_4 =0 ,
	.change_current_parameter_5 =0 ,
	.change_current_parameter_6 =0 ,
...
#if AUTO_CHANGE_CHARGING_CURRENT
static int msm_battery_psy_set_property(struct power_supply *psy,
		enum power_supply_property psp,
		 const union power_supply_propval *val)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		change_max_current = val->intval;
		printk("change_max_current=%d\n",val->intval);
		break;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_POWER_NOW:		
		val->intval = msm_battery_info.vchg_voltage;		
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = msm_battery_info.charge_current;
		break;
	case POWER_SUPPLY_PROP_MANUFACTURER:
			if (msm_battery_info.battery_id== POWER_SUPPLY_MANUFACTURER_TURE)
			val->strval = "CELLON";
		else if(msm_battery_info.battery_id== POWER_SUPPLY_MANUFACTURER_UNKNOWN)
            val->strval = "UNKNOWN";
		  else
			val->strval = "FULSE";
		break;
...
static struct power_supply msm_psy_battery = {
	...
	#if AUTO_CHANGE_CHARGING_CURRENT
	.set_property = msm_battery_psy_set_property,
	#endif
};
...
static int msm_battery_get_charger_status(void)
{
	...
	if (rc < 0) {
		...
		be32_to_cpu_self(reply_charger.vchg_voltage);
		be32_to_cpu_self(reply_charger.charge_current);
		be32_to_cpu_self(reply_charger.battery_id);
...
#if AUTO_CHANGE_CHARGING_CURRENT
//added for RPC transfer the parameter from AP to BP---arvin
static int msm_battery_parametery_arg_func(struct msm_rpc_client *battery_client,
					 void *buffer, void *data)
{
	//struct rpc_reply_charging_parameter *battery_parameter =
	//	(struct rpc_reply_charging_parameter *)data;
           //  msm_battery_psy_set_property();
	u32 *request = (u32 *)buffer;
	int size = 0;
printk("-----111--change_current_parameter_1=%d\n",rpc_reply_charging_parameter.change_current_parameter_1);

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_1);
	size += sizeof(u32);
	request++;

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_2);
	size += sizeof(u32);
	request++;

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_3);
	size += sizeof(u32);
	request++;

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_4);
	size += sizeof(u32);
	request++;

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_5);
	size += sizeof(u32);
	request++;

	*request = cpu_to_be32(rpc_reply_charging_parameter.change_current_parameter_6);
	size += sizeof(u32);

	return size;
}
static int msm_battery_parameter_write(void)
{	int rc;
       struct rpc_reply_charger parameter;
	rc = msm_rpc_client_req(msm_battery_info.battery_client,
				ONCRPC_VBATT_PARAMETER_WRITE_PROC,
				msm_battery_parametery_arg_func, &parameter,
				NULL, NULL,
				msecs_to_jiffies(RPC_TIMEOUT));
	if (rc < 0) {
...
//	pr_debug("BATT: msm_battery_update_psy_status");
	...
	if (msm_battery_info.fuel_gauge)
	{
		...
		vchg_voltage            = reply_charger.vchg_voltage;
		charge_current         =reply_charger.charge_current;
		battery_id                =reply_charger.battery_id;
...
/*	pr_debug("BATT: received, %d, %d, 0x%x; %d, %d, %d, %d; %d, %d, %d; %d, %d, %d\n",
	...
	if (charger_status	== msm_battery_info.charger_status &&
	    ...
	    vchg_voltage        == msm_battery_info.vchg_voltage&&
	    charge_current     == msm_battery_info.charge_current &&
	    battery_id            == msm_battery_info.battery_id &&
...
/*	pr_debug("BATT: received, %d, %d, 0x%x; %d, %d, %d, %d; %d, %d, %d,charge_current=%d,,battery_id=%d,vchg=%d\n",
		  charger_status, charger_hardware, hide,
		  battery_status, battery_voltage, battery_capacity, battery_temp,
		  is_charging, is_charging_complete, is_charging_failed,charge_current,battery_id,vchg_voltage);*/
	...
	#if AUTO_CHANGE_CHARGING_CURRENT	
	if (msm_battery_info.charger_hardware==0){
	if (change_max_current<=450)         
		rpc_reply_charging_parameter.change_current_parameter_1= change_max_current;
		}else if (msm_battery_info.charger_hardware==1){
			if (change_max_current<=650)         
			rpc_reply_charging_parameter.change_current_parameter_1= change_max_current;
		}else if (msm_battery_info.charger_hardware==2){
			if (change_max_current<=450)         
			rpc_reply_charging_parameter.change_current_parameter_1= change_max_current;
		}else{
		rpc_reply_charging_parameter.change_current_parameter_1=msm_battery_info.charge_current;
			}
	printk("rpc_reply_charging_parameter->a1=%d",rpc_reply_charging_parameter.change_current_parameter_1);
		  msm_battery_parameter_write();      
	...
	msm_battery_info.vchg_voltage       = vchg_voltage;
	msm_battery_info.charge_current    = charge_current;
	msm_battery_info.battery_id            = battery_id;
...
{	
	...
	if (disable_charging_flag){
		rc= msm_rpc_call_reply(msm_battery_info.charger_endpoint,
					CHG_CHARGING_DISABLE_PROC,
...
static ssize_t show_charge_control(struct device_driver *driver,  char *buf)
{
